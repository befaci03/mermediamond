...
minifiedMap = false
if (http ~= nil) then local res = http.get(repoBase.."/minified/.min.map")
...
if (map ~= false and map[repoPath] ~= nil) then shell.run("wget "..repoBase.."/minified/"..map[repoPath].." "..localPath) end
if (not fs.exists(localPath)) then shell.run("wget "..repoBase.."/"..repoPath.." "..localPath) end
if (fs.exists(localPath)) then return true end
print("WARNING: failed to download '"..repoPath.."'")
return false end
term.clear()
term.setCursorPos(1,1)
if (fs.exists("/startup.lua") and fs.exists("/ver") and fs.isDir("/lib") and fs.isDir("/server")) then shell.run("/startup.lua")
return end
...
fs.delete("/lib")
fs.makeDir("/lib")
for _, f in ipairs({"bankapi.lua", "uilib.lua", "net.lua", "cards.lua"}) do if (not installFile("lib/"..f, "/lib/"..f)) then ok = false end end
if (ok) then print("Downloading language files...")
fs.delete("/lang")
fs.makeDir("/lang")
...
installFile("server/menu.lua", "/server/menu.lua") end
