...
minifiedMap = false
if (http ~= nil) then local res = http.get(baseUrl.."/minified/.min.map")
if (res ~= nil) then local content = res.readAll()
res.close()
local map = textutils.unserialiseJSON(content)
if (map ~= nil) then local bySource = {}
for minName, srcPath in pairs(map) do bySource[srcPath] = minName end
minifiedMap = bySource end end end
return minifiedMap end
local function mfGet(repoPath, localPath)
fs.delete(localPath)
local map = getMinifiedMap()
if (map ~= false and map[repoPath] ~= nil) then shell.run("wget "..baseUrl.."/minified/"..map[repoPath].." "..localPath) end
if (not fs.exists(localPath)) then shell.run("wget "..baseUrl.."/"..repoPath.." "..localPath) end
return fs.exists(localPath) end
function checklistItem(text, currentAmount, amountNeeded)
local check = "[ ] "
if (currentAmount >= amountNeeded) then check = "[x] " end
print(check..text..": "..currentAmount.."/"..amountNeeded)
return currentAmount >= amountNeeded end
term.clear()
...
while (turtle.detectDown()) do if (peripheral.getType("bottom") == "drive") then term.clear()
term.setCursorPos(1,1)
...
return end
if (not turtle.digDown()) then term.clear()
term.setCursorPos(1,1)
print("=== ATM Setup cancelled ===")
print("")
...
turtle.turnRight()
return clear end
if (not surroundingsClear()) then term.clear()
term.setCursorPos(1,1)
print("=== ATM Setup cancelled ===")
print("")
...
while (turtle.getFuelLevel() < requiredFuel) do term.clear()
term.setCursorPos(1,1)
...
print("")
print("Refuelling...")
for i=1, 16 do if (turtle.getFuelLevel() < requiredFuel) then while (turtle.getItemCount(i) > 0) do turtle.select(i)
if (turtle.refuel(1)) then if (turtle.getFuelLevel() >= requiredFuel) then break end end end end end
sleep(0.1)
if (turtle.getFuelLevel() < requiredFuel) then print("I still don't have enough fuel...")
sleep(2)
else print("All fueled up!")
sleep(1) end end
if (not turtle.down()) then term.clear()
term.setCursorPos(1,1)
print("=== ATM Setup cancelled ===")
print("")
...
if (item ~= nil) then if (item.name == "minecraft:chest") then chests = chests + item.count
...
elseif (item.name == "computercraft:disk") then disks = disks + item.count end end end
local leftEquipment = peripheral.getType("left")
local rightEquipment = peripheral.getType("right")
if (leftEquipment == "workbench" or rightEquipment == "workbench") then craftingTables = craftingTables + 1 end
if (leftEquipment == "modem" or rightEquipment == "modem") then modems = modems + 1 end
term.clear()
term.setCursorPos(1,1)
print("What I need:")
print("")
...
if event == "turtle_inventory" or event == "peripheral" or event == "peripheral_detach" then break end end end
function selectItem(itemName)
while (true) do for i=1, 16 do local item = turtle.getItemDetail(i)
if (item ~= nil) then if (item.name == itemName) then turtle.select(i)
return true end end end
print("Missing item... please insert a '"..itemName.."' in my inventory to continue.")
os.pullEvent("turtle_inventory") end end
function place()
while (not turtle.place()) do print("Something is blocking the way in front of me")
sleep(1) end end
function placeUp()
while (not turtle.placeUp()) do print("Something is blocking the way above me")
sleep(1) end end
function placeDown()
while (not turtle.placeDown()) do print("Something is blocking the way below me")
sleep(1) end end
function forward()
while (not turtle.forward()) do print("Something is blocking the way in front of me")
sleep(1) end end
function up()
while (not turtle.up()) do print("Something is blocking the way above me")
sleep(1) end end
function down()
while (not turtle.down()) do print("Something is blocking the way below me")
sleep(1) end end
function back()
while (not turtle.back()) do print("Something is blocking the way behind me")
sleep(1) end end
term.clear()
term.setCursorPos(1,1)
print("Let's begin!")
...
if (not hasModem) then selectItem("computercraft:wireless_modem_normal")
turtle.equipLeft() end
...
if (not hasWorkbench) then selectItem("minecraft:crafting_table")
turtle.equipRight() end
back()
down()
selectItem("computercraft:disk_drive")
while (not turtle.place()) do if (not turtle.dig()) then print("The spot in front of/below me is blocked and unbreakable!")
sleep(2) end end
selectItem("computercraft:disk")
while (not turtle.place()) do print("Could not insert the floppy disk into the disk drive...")
print("(Make sure the drive is empty and the disk is a data disk.)")
sleep(2) end
local diskPath = disk.getMountPath("front")
if (diskPath == nil) then print("WARNING: can't read the floppy disk I just inserted!")
print("The ATM Terminal will have to be installed manually")
print("(use the installer's 'Install ATM Terminal' option).")
sleep(3)
else print("Writing the ATM Terminal setup to the floppy disk...")
for _, stale in ipairs({"startup.lua", "installer.lua", "autosetup.lua"}) do if (fs.exists(diskPath.."/"..stale)) then fs.delete(diskPath.."/"..stale) end end
mfGet("atm/bootstrap.lua", diskPath.."/startup.lua")
if (fs.exists(diskPath.."/startup.lua")) then print("Setup disk ready!")
else print("WARNING: failed to write the setup disk (no internet?)")
print("The ATM Terminal will have to be installed manually.") end
sleep(2) end
up()
...
for _, lang in ipairs(languages) do mfGet("lang/"..lang..".json", "/lang/"..lang..".json") end
if (fs.exists("lang/languages.lua")) then print("Language files installed!")
else print("WARNING: language files failed to download.")
print("The ATM will show raw translation keys instead of text.") end
...
mfGet("atm/startup.lua", "/startup.lua")
