-- Bank Server admin menu (settings UI)
-- Loaded by server/startup.lua with dofile(); uses the config/db
-- globals from server/config.lua and server/db.lua.
	...
	while true do
		while true do
			...
			elseif (command == "set_owner") then
				...
				-- account ids are strings now (xxx-xxxxxxxxxxx)
